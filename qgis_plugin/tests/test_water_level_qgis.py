"""Tests for water_level_qgis.py (NOAA CO-OPS lookups), mirroring
tests/test_water_level.py test-for-test. All network calls are mocked --
nothing here makes a live HTTP request, matching the rest of this suite's
offline/deterministic convention even though the module under test is the
one exception in the plugin that calls out to a live API in normal use.

The two `water-levels` CLI subcommand tests from the standalone suite are
omitted here: this plugin has no CLI (task #73's scope is just the module
port -- a hybrid dialog + QgsProcessingAlgorithm, not a CLI, is what wires
this module into the plugin UI, per tasks #74-76), so there is no
`_run_water_levels` equivalent to test.
"""
...
def test_site_lat_lon_reprojects_to_wgs84(tmp_path):
    ...
    layer = QgsVectorLayer(str(path), "site_point", "ogr")
    assert layer.isValid()
    lat, lon = wl.site_lat_lon(layer)
