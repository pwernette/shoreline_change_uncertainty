"""Tests for the QGIS-native port of rate_of_change.py, run against the
shapely-backed qgis stub. Mirrors tests/test_rate_of_change.py's
expectations exactly for the four pandas-only functions (no geometry
involved), and adapts build_rate_change_polygons's assertions to read a
QgsVectorLayer instead of a GeoDataFrame.
"""
...
def _two_adjacent_transects(x_a=0.0, x_b=10.0, transect_id_a=0, transect_id_b=1):
    ...
    return [
        Transect(transect_id=transect_id_a, geometry=_line([(x_a, 0), (x_a, 100)]), baseline_point=(x_a, 0)),
        Transect(transect_id=transect_id_b, geometry=_line([(x_b, 0), (x_b, 100)]), baseline_point=(x_b, 0)),
    ]
...
def test_build_rate_change_polygons_basic_geometry_and_attributes():
    ...
    feat = out.getFeatures()[0]
    assert feat["TRANSECT_A"] == 0
    assert feat["TRANSECT_B"] == 1
    assert feat["YEAR_A"] == 2000
    assert feat["YEAR_B"] == 2010
    # MAGNITUDE = mean((25-20), (30-20)) = mean(5, 10) = 7.5; same sign
    # convention as EPR_NET_DISTANCE (positive here = accretion).
    assert feat["MAGNITUDE"] == pytest.approx(7.5)
    assert feat["RATE"] == pytest.approx(0.75)  # 7.5 / (2010 - 2000)
    # 7.5m of separation with sigma=1 on each side is overwhelmingly "real".
    assert feat["PROB_CHANGE"] > 0.99
    # Quadrilateral bounded by (0,20)-(0,25)-(10,30)-(10,20): a trapezoid
    # with parallel sides of length 5 and 10, 10m apart.
    assert feat.geometry().area() == pytest.approx(75.0)
    assert out.crs().isValid()
...
def test_build_rate_change_polygons_skips_nonsequential_transect_ids():
    ...
    transects = [
        ...
        Transect(transect_id=3, geometry=_line([(30, 0), (30, 100)]), baseline_point=(30, 0)),
    ]
    wide = pd.DataFrame({
        "TRANSECT_ID": [0, 2, 3],
    ...
    assert out.featureCount() == 1
    feat = out.getFeatures()[0]
    assert [feat["TRANSECT_A"], feat["TRANSECT_B"]] == [2, 3]
...
def test_build_rate_change_polygons_skips_missing_to_values():
    ...
    assert set(["TRANSECT_A", "TRANSECT_B", "YEAR_A", "YEAR_B", "MAGNITUDE", "RATE", "PROB_CHANGE"]) <= set(
        out.fields().names()
    )
...
def test_build_rate_change_polygons_multiple_year_pairs():
    ...
    pairs = {(f["YEAR_A"], f["YEAR_B"]) for f in out.getFeatures()}
