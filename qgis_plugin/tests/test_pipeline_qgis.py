...
from conftest.py, run the full QGIS-native pipeline, and confirm every
expected output (CSV tables, shapefiles, GeoTIFF rasters) is actually
written to disk. Mirrors tests/test_pipeline.py (the standalone package's
equivalent) test-for-test; output file names/columns are identical since
pipeline_qgis.py was ported to write the exact same outputs.

Most assertions read output shapefiles via plain `geopandas.read_file` --
they're real on-disk shapefiles regardless of which pipeline wrote them, and
these checks only need attribute/column values, not QgsGeometry-specific
behavior. The one exception is the MAGNITUDE-column test, which must
reconstruct `Transect` objects with real `QgsGeometry` geometries (via
`io_utils_qgis.read_shoreline`) so that `transects_qgis.nearest_transect_net_distance`'s
`point.distance(t.geometry)` call works -- see that test's docstring.
"""
...
def test_run_pipeline_prob_change_segments_includes_magnitude_column(synthetic_site, tmp_path):
    ...
    from transects_qgis.nearest_transect_net_distance against the *general*
    ...
    indicator (see probability_surface_qgis.py's module docstring). This test
    recomputes the expected value independently from transects.shp +
    transect_distances_wide.csv and confirms it matches what pipeline_qgis.py
    wrote to each segment.

    Reads transects.shp/the segment shapefiles via io_utils_qgis.read_shoreline
    (not plain geopandas) so that every geometry handed to
    nearest_transect_net_distance is a real QgsGeometry -- that function's
    `point.distance(t.geometry)` call requires both sides to be QgsGeometry
    under the qgis_stub, same as it would against the real qgis.core API.
    """
    ...
    site_dir = output_dir / "synthetic_site"
    transects_layer = io_utils.read_shoreline(site_dir / "transects.shp")
    wide = pd.read_csv(site_dir / "transect_distances_wide.csv")
    # ESRI Shapefile driver truncates TRANSECT_ID (11 chars) -> TRANSECT_I.
    field_names = transects_layer.fields().names()
    id_col = "TRANSECT_I" if "TRANSECT_I" in field_names else "TRANSECT_ID"
    transects = [
        Transect(transect_id=int(feat[id_col]), geometry=feat.geometry(), baseline_point=(0.0, 0.0))
        for feat in transects_layer.getFeatures()
    ]
    ...
    for i, year_a in enumerate(years):
        for year_b in years[i + 1:]:
            for suffix in (f"{year_a}_vs_{year_b}", f"{year_b}_vs_{year_a}"):
                seg_layer = io_utils.read_shoreline(site_dir / f"change_probability_segments_{suffix}.shp")
                assert "MAGNITUDE" in seg_layer.fields().names()
                for feat in seg_layer.getFeatures():
                    seg_geom = feat.geometry()
                    # Absolute-distance QgsGeometry.interpolate convention
                    # (not shapely's normalized=True) -- see module docstrings.
                    midpoint = seg_geom.interpolate(0.5 * seg_geom.length())
                    expected = nearest_transect_net_distance(midpoint, transects, wide, year_a, year_b)
                    magnitude = feat["MAGNITUDE"]
                    if pd.isna(expected):
                        assert pd.isna(magnitude)
                    else:
