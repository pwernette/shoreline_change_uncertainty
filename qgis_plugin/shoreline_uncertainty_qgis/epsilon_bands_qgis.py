...
same way -- just built on `qgis.core.QgsGeometry` (GEOS-backed, like
shapely) instead of shapely directly:

1. Overlapping Double Buffer (ODB) -- the PUBLISHED method from Wernette et
   al. (2017): Ps = Area(buffer_a INTERSECT buffer_b) / Area(buffer_a UNION
   buffer_b). `significant_change = Ps < threshold`.
2. Iterative buffer-growth ("Perkal-style" legacy method) -- an UNPUBLISHED
   alternative. NOT the method described in the published paper; kept
   side by side for completeness/comparison, never merged with #1.
"""
...
# Default circle-approximation segments-per-quarter-circle for
# QgsGeometry.buffer(), matching shapely's default quad_segs=8 so buffer
# areas/behavior line up with the standalone package's numbers.
_DEFAULT_BUFFER_SEGMENTS = 8
...
class ODBResult:
    ...
    raster_output_qgis.build_similarity_surface can rasterize them directly
    ...
    buffer_a: QgsGeometry
    buffer_b: QgsGeometry
...
def overlapping_double_buffer(
    ...
    *,
    ...
    buffer_segments: int = _DEFAULT_BUFFER_SEGMENTS,
) -> ODBResult:
    """Core ODB computation for a single shoreline pair (Eq. 4, Wernette et al. 2017)."""
    buffer_a = geom_a.buffer(radius_a, buffer_segments)
    buffer_b = geom_b.buffer(radius_b, buffer_segments)
    area_a = buffer_a.area()
    area_b = buffer_b.area()
    intersection = buffer_a.intersection(buffer_b)
    union = buffer_a.combine(buffer_b)
...
def grow_buffer_to_threshold(
    shoreline: QgsGeometry,
    adjacent_shoreline: QgsGeometry,
    confidence_level: float,
    *,
    step: float = 0.5,
    ...
    while obs_length < threshold:
        buf = shoreline.buffer(bufdist, buffer_segments)
