...
horizontal shoreline position instead of vertical elevation:
  ...
  - Per-pixel, continuous raster (`change_probability_raster`), "around
    ...
    convention transects_qgis.py uses), so that distance differences
    between two shorelines correctly capture cases where a point lies
    between them, not just how far each line is from it. sigma is constant
    per shoreline year -- this package has no spatially-variable
    uncertainty *surface* for shoreline vectors, unlike the DSM uncertainty
    rasters in the 2020 paper, so sigma does not vary pixel-to-pixel within
    a year.
...
Ported onto `qgis.core.QgsGeometry` + the QGIS-bundled-GDAL-backed
raster_output_qgis.py instead of shapely + rasterio:
  - `_iter_simple_lines`/`segment_line` use `QgsGeometry.mergeLines()`
    (GEOS linemerge, same engine shapely.ops.linemerge wraps) +
    `asGeometryCollection()` to decompose a (multi)line into simple parts,
    and the new `geometry_utils_qgis.substring` helper (vertex-walking +
    `interpolate()`, since a guaranteed curve-substring QgsGeometry/QgsCurve
    method isn't available across the whole supported QGIS version range)
    in place of shapely.ops.substring.
  - `shoreline_change_probability_segments` builds a QgsVectorLayer (via
    io_utils_qgis.build_memory_layer) instead of a GeoDataFrame.
  - scipy (ndimage/stats) and pandas have no spatial dependency and are
    reused as-is, same as elsewhere in this plugin.
"""
...
from .geometry_utils_qgis import substring
from .io_utils_qgis import _to_qgis_crs, build_memory_layer
from .raster_output_qgis import GridTransform, rasterize_geometry
...
def signed_distance_raster(
    ...
    convention transects_qgis.py establishes via coordinate_priority,
    applied here pixel-wise so distances to two different shorelines can be
...
def change_probability_raster(
    line_a: QgsGeometry, sigma_a: float,
    line_b: QgsGeometry, sigma_b: float,
    baseline_center: Tuple[float, float],
    baseline_direction: Tuple[float, float],
    transform: GridTransform, width: int, height: int,
) -> Tuple[np.ndarray, np.ndarray]:
...
def _iter_simple_lines(geom: QgsGeometry) -> List[QgsGeometry]:
    """Flatten a LineString/MultiLineString geometry (e.g. the
    geometry_utils_qgis.dissolve of a shoreline layer's features) into a
    flat list of simple-part QgsGeometry lines, merging contiguous
    MultiLineString parts back into single lines where possible
    (`QgsGeometry.mergeLines()`, GEOS-backed -- same engine
    shapely.ops.linemerge wraps) so segmenting doesn't introduce spurious
    breaks at original feature boundaries."""
    if geom is None or geom.isEmpty():
        return []
    if QgsWkbTypes.geometryType(geom.wkbType()) != QgsWkbTypes.LineGeometry:
        raise ValueError(f"Unsupported geometry type for segmentation: {geom.wkbType()}")
    working = geom.mergeLines() if geom.isMultipart() else geom
    if working.isEmpty():
        return []
    return working.asGeometryCollection()


def segment_line(line: QgsGeometry, segment_length: float) -> List[QgsGeometry]:
    ...
    flat list of QgsGeometry line segments, in order, covering the whole
    input geometry."""
    if segment_length <= 0:
    ...
    for part in _iter_simple_lines(line):
        ...
        for i in range(n_segments):
            ...
            if not sub.isEmpty() and sub.length() > 0:
                segments.append(sub)
    return segments


def _shrink_for_rasterize_mask(seg: QgsGeometry, frac: float = 1e-6) -> QgsGeometry:
    """Trim a tiny epsilon off both ends of `seg` before rasterizing it for a
    cell-membership mask, to dodge GDAL's `RasterizeLayer`/rasterio's
    ...
    line geometries regardless of the all-touched setting), a segment whose
    endpoint sits precisely on a column/row edge can get burned into the
    neighboring cell as well as the intended one (confirmed via a direct
    rasterize() repro on the equivalent rasterio code path -- e.g. a 10m
    grid with two 10-wide columns: a line from (0,5) to (10,5) burns into
    both columns, while the adjacent (10,5)-(20,5) segment burns into only
    the second). Since `segment_line` deliberately produces segments whose
    endpoints fall exactly on multiples of `segment_length` -- which line up
    with cell edges whenever segment_length is a multiple of the grid's
    cell size -- this is not a rare edge case for this caller. Shrinking
    each endpoint inward by `frac` of the segment's own length (floor of
    1e-9 map units) moves it just off the boundary without changing which
    cell(s) a segment that genuinely straddles a cell boundary mid-segment
    is counted in."""
    length = seg.length()
    if length <= 0:
        return seg
...
def shoreline_change_probability_segments(
    ...
    width: int,
    height: int,
    crs: Optional[str | int | QgsCoordinateReferenceSystem] = None,
    ...
    typically compute this via `transects_qgis.nearest_transect_net_distance`
    ...
    Returns a QgsVectorLayer with fields SEG_ID, LENGTH, PROB_CHANGE
    (, MAGNITUDE) -- empty (but correctly shaped) if `line` has no length.
    """
    qgis_crs = (
        crs
    ...
    fields = [QgsField("SEG_ID"), QgsField("LENGTH"), QgsField("PROB_CHANGE")]
    if magnitudes is not None:
        if len(magnitudes) != len(segments):
            raise ValueError(
                f"magnitudes has {len(magnitudes)} entries but {len(segments)} segments were generated."
            )
        fields.append(QgsField("MAGNITUDE"))

    attrs = []
    for i, (seg, mean) in enumerate(zip(segments, means)):
        row = [i, seg.length(), mean]
        if magnitudes is not None:
            row.append(magnitudes[i])
    ...
    return build_memory_layer(
        ...
        crs=qgis_crs,
        name="change_probability_segments",
    )
...
def change_probability_table(wide_table: pd.DataFrame, sigma_by_year: Dict[int, float]) -> pd.DataFrame:
    ...
    existing transect_distances_wide table (transects_qgis.to_wide_table),
