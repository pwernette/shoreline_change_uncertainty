...
def test_compare_to_professionals_basic():
    primary = LineString([(0, 0), (100, 0)])
    profs = {
        "acmoody": LineString([(0, 1), (100, 1)]),
        "goodwin": LineString([(0, -2), (100, -2)]),
    }
    df = compare_to_professionals("test", 2000, primary, profs)
    assert len(df) == 2
    assert set(df["TO_PROF"]) == {"acmoody", "goodwin"}
    assert (df["FROM"] == "PRIMARY").all()
    # acmoody is 1m away, goodwin is 2m away -> distances should reflect that.
...
def test_compare_professionals_pairwise_excludes_self_pairs():
    profs = {
        "acmoody": LineString([(0, 0), (100, 0)]),
        "goodwin": LineString([(0, 1), (100, 1)]),
        "lusch": LineString([(0, -1), (100, -1)]),
    }
    df = compare_professionals_pairwise("test", 2000, profs)
    # 3 professionals, ordered pairs excluding self -> 3*2 = 6 rows
    assert len(df) == 6
